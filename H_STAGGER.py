...
# Calculating the differences between each pair of TOAs
differences = pdist(A[:, None])

# Limit differences
differences = differences[differences < 4000]

# Binning the differences
binWidth = 0.001 * 10000
edges = np.arange(0, 10000 + binWidth, binWidth)
histCounts, _ = np.histogram(differences, edges)

# Searching for peaks in the histogram
peaks, _ = find_peaks(histCounts, prominence=3, height=3)

PRIs = edges[peaks] + binWidth / 2

# Select PRIs up to the maximum peak
STAGGER_PRIs = PRIs[:np.where(histCounts == np.max(histCounts))[0][0] + 1]

# Check for sum ts combination
P_STAGGER = []

# Test for 2 elements
if len(STAGGER_PRIs) >= 3:
    for i in range(len(STAGGER_PRIs) - 2):
        for j in range(i + 1, len(STAGGER_PRIs) - 1):
            sum_vals = STAGGER_PRIs[i] + STAGGER_PRIs[j]
            if abs(sum_vals - STAGGER_PRIs[-1]) <= 5:
                P_STAGGER.append([STAGGER_PRIs[i], STAGGER_PRIs[j]])

# Test for 3 elements
if len(STAGGER_PRIs) >= 4:
    for i in range(len(STAGGER_PRIs) - 3):
        for j in range(i + 1, len(STAGGER_PRIs) - 2):
            for k in range(j + 1, len(STAGGER_PRIs) - 1):
                sum_vals = STAGGER_PRIs[i] + STAGGER_PRIs[j] + STAGGER_PRIs[k]
                if abs(sum_vals - STAGGER_PRIs[-1]) <= 5:
                    P_STAGGER.append([STAGGER_PRIs[i], STAGGER_PRIs[j], STAGGER_PRIs[k]])

# Test for 4 elements
if len(STAGGER_PRIs) >= 5:
    for i in range(len(STAGGER_PRIs) - 4):
        for j in range(i + 1, len(STAGGER_PRIs) - 3):
            for k in range(j + 1, len(STAGGER_PRIs) - 2):
                for l in range(k + 1, len(STAGGER_PRIs) - 1):
                    sum_vals = STAGGER_PRIs[i] + STAGGER_PRIs[j] + STAGGER_PRIs[k] + STAGGER_PRIs[l]
                    if abs(sum_vals - STAGGER_PRIs[-1]) <= 5:
                        P_STAGGER.append([STAGGER_PRIs[i], STAGGER_PRIs[j], STAGGER_PRIs[k], STAGGER_PRIs[l]])

# Display results
print("Found STAGGER PRIs emitter:")
print(np.array(P_STAGGER))

# Visualization
plt.bar(edges[:-1], histCounts, align='edge', width=binWidth)
plt.scatter(PRIs, histCounts[peaks], color='red', marker='o')
plt.xlabel('TOA Difference')
plt.ylabel('Count')
plt.xlim([0, 4000])
plt.title('Histogram of TOA Differences with Detected PRIs')
plt.show()
