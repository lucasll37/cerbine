...
# MANUALMENTE - 'buffer' o buffer de pulsos
# buffer = []  # Inicialize buffer como uma lista vazia
...
# buffer = np.round(BUFFER)
# buffer = buffer[4:15, 0:14]
                ...
                if proximos_PPI_condizentes:
                    ...
                    for q in range(len(proximos_PPI_condizentes)):
                        ...
                        if sum(np.array(contador) != 0) >= 2:
                            ...
                            TREM = [buffer[indice_buffer], np.mean([x for x in PRI if x != 0]), np.mean([x for x in DEV if x != 0])]
